...
// export const createCourse = catchAsyncError(async (req, res, next) => {
//    const { title, description, category, createdBy, file } = req.body;
...
export const addLectures = catchAsyncError(async (req, res, next) => {
   ...
   res.status(200).json({
      success: true,
      lectures: 'Lectures Added Successfully',
   });
});
...
Course.watch().on('change', async () => {
   ...
   if (!latestStats || latestStats.views !== newStats.views) {
      await newStats.save();
   }
});
